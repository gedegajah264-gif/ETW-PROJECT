--[[
    HAZE V2.2 - CYAN NEON (SECURITY UPDATE)
    - FIXED: Message kick ganti jadi "You are not whitelisted on HAZE"
    - FIXED: cariDataBruteForce restored (ESP No longer errors)
    - FIXED: Attack TP & Target System restored
]]
...
-- SECURITY: WHITELIST
-- ==========================================
local Whitelist = {"hanssaputra15", "L0rdDarkShad0w", "chibhothax"}
...
if not isWhitelisted then 
    LocalPlayer:Kick("You are not whitelisted on HAZE") 
    return 
end
...
local function createESP(p)
    ...
    espObjects[p] = {highlight = hl, billboard = bg, label = tl}
end
