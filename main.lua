--[[
    HAZE V2.2 - CYAN NEON (SECURITY + EXECUTOR BYPASS)
    - NEW: Whitelist System (hanssaputra15, L0rdDarkShad0w, chibhothax)
...
if not isWhitelisted then
    LocalPlayer:Kick("Akses Ditolak! Gak diajak, Ze! (Whitelisted Only)")
...
-- GRAVITY & GOD MODE (SAMA SEPERTI SEBELUMNYA)
        ...
        if espEnabled then
            for _, obj in ipairs(espObjects) do
                ...
                if char and char:FindFirstChild("HumanoidRootPart") then
                    ...
                    local sizeVal = cariDataBruteForce(obj.player, "Size")
                    obj.textLabel.Text = string.format("[%s]\nSize: %s", obj.player.Name, tostring(sizeVal))
                else
...
createToggle("Player ESP", false, function(v) 
    espEnabled = v 
    if not v then
        for _, obj in ipairs(espObjects) do obj.highlight.Parent = nil obj.billboard.Parent = nil end
    end
end)
